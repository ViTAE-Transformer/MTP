...
from mmrotate.registry import MODELS as ROD_MODELS
...
def unpack_r_gt_instances(batch_data_samples: SampleList) -> tuple:
    ...
    for data_sample in batch_data_samples:
        ...
        batch_gt_instances.append(data_sample.r_gt_instances)
        if 'r_ignored_instances' in data_sample:
            batch_gt_instances_ignore.append(data_sample.r_ignored_instances)
        else:
...
class MTP_RD_StandardRoIHead(BaseRoIHead):
    ...
    def init_bbox_head(self, bbox_roi_extractor: ConfigType,
        ...
        """
        self.bbox_roi_extractor = ROD_MODELS.build(bbox_roi_extractor)
    ...
    def gen_sampling_results(self, tbr) -> dict:

        x, rpn_results_list, batch_data_samples = tbr

        assert len(rpn_results_list) == len(batch_data_samples)
        outputs = unpack_r_gt_instances(batch_data_samples)
    ...
    def train_bbox_forward(self, x: Tuple[Tensor], 
        ...
        return bbox_feats, rois


    def test_bbox_generate_roi(self, rpn_results_list):
    ...
    def test_bbox_roi_feats(self, x, rois):
    ...
    def bbox_loss(self, num_classes, sampling_results, rois, bbox_results) -> dict:

...
def samplelist_boxtype2tensor(batch_data_samples: SampleList) -> SampleList:
    for data_samples in batch_data_samples:
        if 'r_gt_instances' in data_samples:
            ...
            labels = data_samples.r_gt_instances.get('labels', None)
            if isinstance(bboxes, BaseBoxes):
                data_samples.r_gt_instances.bboxes = bboxes.tensor
            data_samples.r_gt_instances.labels = to_tensor(labels)

        if 'r_pred_instances' in data_samples:
            bboxes = data_samples.r_pred_instances.get('bboxes', None)
            labels = data_samples.r_pred_instances.get('labels', None)
            if isinstance(bboxes, BaseBoxes):
                data_samples.r_pred_instances.bboxes = bboxes.tensor
            data_samples.r_pred_instances.labels = to_tensor(labels)

        if 'r_ignored_instances' in data_samples:
            bboxes = data_samples.r_ignored_instances.get('bboxes', None)
            labels = data_samples.r_ignored_instances.get('labels', None)
            if isinstance(bboxes, BaseBoxes):
                data_samples.r_ignored_instances.bboxes = bboxes.tensor
            data_samples.r_ignored_instances.labels = to_tensor(labels)

class MTP_RD_BaseDetector(BaseModel, metaclass=ABCMeta):
    ...
    def add_pred_to_datasample(self, data_samples: SampleList,
        ...
        for data_sample, pred_instances in zip(data_samples, results_list):
            data_sample.r_pred_instances = pred_instances

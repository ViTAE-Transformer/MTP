...
class Attention(nn.Module):
    ...
    def forward(self, x, H, W, rel_pos_bias=None):
        ...
        attn = calc_rel_pos_spatial(attn, q, self.window_size, self.window_size, self.full_attn_rel_pos_h, self.full_attn_rel_pos_w)
...
class RVSA_MTP(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=80, embed_dim=768, depth=12,
                 ...
                 out_indices=[11], interval=3, pretrained=None, restart_regression=True, frozen_stages=-1):
        super().__init__()
...
def vit_b_rvsa(args, inchannels=3):

    backbone = ViT_Win_RVSA_V3_WSZ7(
...
def vit_l_rvsa(args, inchannels=3):
